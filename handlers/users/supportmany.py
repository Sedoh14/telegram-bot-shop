                ...
                if call.from_user.username:
                    await bot.send_message(chat_id=contact_ids[i],
                                           text=f"С вами хочет связаться пользователь {call.from_user.full_name}, "
                                                f"c username: {call.from_user.username}",
                                           reply_markup=keyboard1)
                else:
                    await bot.send_message(chat_id=contact_ids[i],
                                           text=f"С вами хочет связаться пользователь {call.from_user.full_name}, "
                                                f"его username отсутствует.",
                                           reply_markup=keyboard1)
    ...
    if str(queue[0])[-1].isdigit():
        ...
        if item.photo:
            ...
            if item.photo.split(',')[0][-5:]=='ВИДЕО':
                await call.message.answer_video(item.photo.split(',')[0][:-5])
            else:
                await call.message.answer_photo(item.photo.split(',')[0])
...
@dp.message_handler(commands=['cancel'], state=['start', 'pre_wait_in_support'])
async def exit1(mess: types.Message, state: FSMContext):
    await state.reset_state()
    await mess.answer('Вы завершили сеанс техподдержки')
...
async def not_supported(message: types.Message, state: FSMContext):
    if await state.get_state() == 'start' or await state.get_state() == 'pre_wait_in_support':
        await message.answer(
            'Нажмите на кнопку "Написать оператору" или отмените вызов путём нажатия на кнопку "Завершить сеанс"'
            ' или путём ввода команды /cancel')
    else:
